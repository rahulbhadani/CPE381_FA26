...
\begin{frame}{Signal Stretching along $x$-axis}
    ...
    \begin{itemize}
    \item $f(x) \to  f(ax); \quad a > 1$ : Shrink the graph of $f(x)$ `$a$' times along x-axis.
    \item $f(x) \to f\bigg(\cfrac{1}{a} x\bigg); \quad a > 1$ : Stretch the graph of $f(x)$ `$a$' times along x-axis.
...
% ---------- grid ----------
\draw[lightgray!40, thin, step=0.5] (-3,-2) grid (3,2);

% ---------- axes ----------
\draw[axisline] (-3.4,0) -- (3.6,0) node[right, black, inner sep=4pt] {$t$};
\draw[axisline] (0,-2.4) -- (0,2.5) node[above, black, inner sep=4pt]
      {$\Lambda_1(t)$};
...
% ---------- signal ----------
\draw[curve] (-3,0) -- (0,0) -- (1,1) -- (3,1);
\foreach \p in {(0,0),(1,1)}
  \fill[green!50!black] \p circle (2.2pt);

% ---------- ticks ----------
\foreach \u in {-3,-2,-1,1,2,3}
...
\foreach \v in {-2,-1.5,-1,-0.5,0.5,1,1.5,2}
  \node[lbl, left=5pt] at (0,\v) {$\v$};
\node[lbl, anchor=north east, xshift=-2pt, yshift=-2pt] at (0,0) {$0$};

% ---------- title above the grid ----------
\node[green!50!black, font=\bfseries, anchor=south] at (0,2.9)
      {$\Lambda_1(t) = r(t) - r(t-1)$};
...
% ---------- signal ----------
\draw[curve] (-3,0) -- (1,0) -- (2,-1) -- (3,-1);
\foreach \p in {(1,0),(2,-1)}
  \fill[green!50!black] \p circle (2.2pt);
...
% ---------- tick labels ----------
\foreach \u in {-3,-2,-1,2,3}
  \node[lbl, below=5pt] at (\u,0) {$\u$};
% the falling edge starts at t=1, so its label steps aside
\node[lbl, below=5pt, xshift=-6pt] at (1,0) {$1$};
\foreach \v in {-2,-1.5,-1,-0.5,0.5,1,1.5,2}
  \node[lbl, left=5pt] at (0,\v) {$\v$};
\node[lbl, anchor=north east, xshift=-2pt, yshift=-2pt] at (0,0) {$0$};
