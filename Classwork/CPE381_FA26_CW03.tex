...
\newcommand{\hw}{Classwork 03: Mathematical Preliminaries}
\newcommand{\duedate}{August 31, 2026}
...
\begin{enumerate}
\item Solve the differential equation $ty'  + 3 y = 0$, given $y(1) = 2$, assuming $t > 0$. Here $y' = \frac{dy}{dt}$. \textbf{Hint 1:} you will need to rearrange terms so that derivative term is on the left hand side and non-derivative terms on the right hand side, and then integrate. \textbf{Hint 2:} $\int \frac{1}{t} dt  = \ln(t) + C$. \hfill \textbf{(5 Points)}
...
\item Find the derivate of $f(x) = x^2$ using the first priciple of the derivate.  \hfill \textbf{(5 Points)}
...
\item Find the second, fourth, seventh, and ninth terms of the geometric progression with first term $a = 2$ and ratio $r = 3$. Does the sum of this geometric progression converge? \textbf{(4 + 1 Points)}
...
\item Find area under the curve  for the shaded region in Figure~\ref{fig:curve}. \textbf{Hint:} $\int x^n dx = \frac{1}{n+1}x^{n+1}$.\hfill \textbf{(5 Points)}
\begin{figure}[htpb]
\centering
\includegraphics[width=0.35\linewidth, trim=0cm 0cm 0cm 0cm,clip]{../Lectures/figures/integration.pdf}
