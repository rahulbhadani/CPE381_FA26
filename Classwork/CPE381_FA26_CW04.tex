...
\newcommand{\hw}{Classwork 04: Discrete and Continuous Representations}
\newcommand{\duedate}{September 02, 2026}
...
\item Consider a signal $f(t)$ where $f(3) = 6$, and $f(5) = 12$. What will be the value of signal at $t = 4$, i.e. $f(4)$ if you happen to employ linear interpolation. \hfill \textbf{(10 Points)}
...
% Linear interpolation line between known points
\addplot[thick, black, mark=*, mark options={solid, black, fill=black}] coordinates {(3,6) (5,12)};

% Labels for known points
\node[anchor=south west] at (axis cs:3,6) {(3, 6)};
\node[anchor=south west] at (axis cs:5,12) {(5, 12)};

% Mark query point at t=4 on x-axis with bold 'x'
% A white background is added behind the node so it sits cleanly over the grid/axis line
\node[font=\bfseries, fill=white, inner sep=1pt] at (axis cs:4,0) {\L0arge x};
...
\item What are some non-ideal scenarios you may encounter while dealing with real-world signals? \hfill \textbf{(5 Points)}
...
\item Consider a MATLAB code snippet for the time vector
...
\begin{enumerate}
\item What is the sampling time period $T_s$? \hfill \textbf{(2.5 Points)}
\item What is the final time ?\hfill \textbf{(2.5 Points)}
\end{enumerate}
